...
# View Own Bookings-------------------------------------------------------------------------------------------
def view_existing_bookings(user):
    print("\nYour Existing Bookings:")
    Booking.view_bookings(user.user_id)
#------------------------------------------------------------------------------------------------------------------

#View own bookings with enter prompt--------------------------------------------------------------------------------
def view_bookings_with_enter(user):
    view_existing_bookings(user)
...
def create_booking(user):
    ...
    while True:
        ...
        if Booking.check_room_availability(room_id, start_datetime, end_datetime):
            booking = Booking.make_booking(room_id, patient_id, start_datetime, end_datetime, user.user_id)
            if booking:
...
def update_booking(user):
    ...
    while True:
        ...
        if option.lower() == 'cancel' or option == '3':
            print("Operation cancelled. Returning to previous menu.")
            break
...
def cancel_booking(user):
    # View own bookings
    bookings = Booking.view_bookings(user.user_id)

    # Input the ID of the booking to cancel
    ...
    # Check if the booking with the entered ID exists
    matching_bookings = [booking for booking in bookings if booking.startswith(str(booking_id))]
    ...
    if confirmation == "yes":
        # Update the status of the booking to "Cancelled" in the file
        updated_bookings = [booking.replace("Confirmed", "Cancelled") if booking.startswith(str(booking_id)) else booking for booking in bookings]
...
#Main Function-----------------------------------------------------------------------------------------------------
def main(user):
    print("Welcome to the Staff Booking Menu")
